...
const lib_buffer = lib(() => {
  /**
  * Assumes `Buffer.TYPED_ARRAY_SUPPORT` === true
  * Use Uint8Array implementation (fastest)
  ...
  */
  ...
  Buffer.byteLength = (string) => string.length >>> 1;
  /**
  ...
  **/
   Buffer.from = function (value, encodingOrOffset, length) {
    length = Buffer.byteLength(value, encodingOrOffset) | 0;
    let that = createBuffer(null, length);
    const actual = that.write(value, encodingOrOffset);
...
const lib_account = lib(() => {
 ...
 (function(Buffer) {
  ...
  const addressFromKey = privateKey => {
    const pubKey = secp256k1.getPublicKey(privateKey.slice(2));
    const pubKeyHash = keccak256(Buffer.from(pubKey.slice(2), 'hex'));
    const address = '0x' + pubKeyHash.slice(-40);
    return addressChecksum(address);
  };

  const signMessage = async (hash, privateKey) => {
    const signature = await secp256k1.sign(hash.slice(2), privateKey.slice(2));
    return signature;
  };

  const signerAddress = (hash, signature) => {
    const recoveredPubkey = secp256k1.recoverPublicKey(hash.slice(2), signature);
    const pubKeyHash = keccak256(Buffer.from(recoveredPubkey.slice(2), 'hex'));
    const address = '0x' + pubKeyHash.slice(-40)
    return addressChecksum(address);
  };
